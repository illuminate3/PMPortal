...
(function($) {
    ...
    function buildNode($node, $appendTo, level, index, opts) {
        ...
        var $adjunct2 = $node.children("adjunct2").eq(0);
        if ($adjunct2.length > 0) {
            var $adjunct2Div = $("<div>").addClass("adjunct2 node").addClass("level"+level).addClass("node"+index).addClass("level"+level+"-node"+index).append(opts.nodeText($adjunct2));
            $adjunct2Div.appendTo($nodeCell);
            var $linkDiv = $("<div>").addClass("adjunct2-link");
            $linkDiv.appendTo($nodeCell);
            $adjunct2.remove();
        }

        var $adjunct3 = $node.children("adjunct3").eq(0);
        if ($adjunct3.length > 0) {
            var $adjunct3Div = $("<div>").addClass("adjunct3 node").addClass("level"+level).addClass("node"+index).addClass("level"+level+"-node"+index).append(opts.nodeText($adjunct3));
            $adjunct3Div.appendTo($nodeCell);
            var $linkDiv = $("<div>").addClass("adjunct3-link");
            $linkDiv.appendTo($nodeCell);
            $adjunct3.remove();
        }
        ...
        if ($childNodes.length > 0) {
            if (opts.depth == -1 || level+1 < opts.depth) {
                var $downLineRow = $("<tr/>").addClass("lines").addClass("linelevel"+level);
