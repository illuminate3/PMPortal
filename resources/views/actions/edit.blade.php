	...
	<div class="row-fluid">
		...
		<div class="span9 pull-right">
			<div class="container panel panel-default full">
				...
				{!! Form::model($action, ['method' => 'PATCH', 'action' => ['ActionsController@update', $action->id]]) !!}
					<div class="panel-body">
						<table class="table-condensed table-hover"> <tbody>
							<tr>
								<td class="span3 right"> {!! Form::label('action_item', 'Action Item: ') !!}</td>
							...
							<tr>
								...
								<td> <select class="span5" name="status" value="{{ old('status') }}">
										<option value="Open" <?php if($action->status == 'Open') {echo("selected");} ?>> 
											Open</option>
										<option value="Closed" <?php if($action->status == 'Closed') {echo("selected");} ?>>
											Closed</option>
									</select> </td>
							</tr>
						...
						<div class = "submit">
							...
							{!! Form::open(['route' => ['actions.destroy', $action->id], 'method' => 'delete', 'class' => 'delete' ]) !!}
